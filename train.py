...
from deepvio.dynamic_model import DeepVIO, DynamicDeepVIOV3
...
def train(par, VIONet, train_dl, test_dl):
    ...
    epochs_warmup, epochs_joint, epochs_fine = 40, 40, 20
    lr_joint, lr_fine = 5e-5, 1e-6
    # epochs_warmup, epochs_joint, epochs_fine = 30, 50, 20
    # lr_joint, lr_fine = 1e-4, 1e-5
...
def main(par):
    ...
    # # Initialize the feature extraction network
    # # Load FlowNet weights pretrained with FlyingChairs
    # pretrained_w = torch.load(par.pretrained_flownet, map_location='cpu')
    # if par.pretrained_flownet and not par.resume:
    #     model_dict = VIONet.visual_encoder.state_dict()
    #     update_dict = {k.replace(".0.", ".conv.conv."): v for k, v in pretrained_w['state_dict'].items() 
    #                 if k.replace(".0.", ".conv.conv.") in model_dict}
    #     model_dict.update(update_dict)
    #     VIONet.visual_encoder.load_state_dict(model_dict)
    # del pretrained_w

    VIONet = VIONet.to(device)

    train_dl, test_dl = dataset(par)
