...
class VisualEncoderFlops(nn.Module):
    def __init__(self, par):
        super(VisualEncoderFlops, self).__init__()
        # CNN
        self.par = par
        self.batchNorm = par.batch_norm
        # searched net with low latency
        self.conv1 = ConvLayer(6, 8, kernel_size=5, stride=2, use_norm=self.batchNorm, norm_func='BN', act_func='lrelu')
        self.conv2 = ConvLayer(8, 16, kernel_size=5, stride=2, use_norm=self.batchNorm, norm_func='BN', act_func='lrelu')
        self.conv3 = ConvLayer(16, 32, kernel_size=3, stride=2, use_norm=self.batchNorm, norm_func='BN', act_func='lrelu')
        self.conv3_1 = ConvLayer(32, 64, kernel_size=3, stride=1, use_norm=self.batchNorm, norm_func='BN', act_func='lrelu')
        self.conv4 = ConvLayer(64, 64, kernel_size=3, stride=2, use_norm=self.batchNorm, norm_func='BN', act_func='lrelu')
        self.conv4_1 = ConvLayer(64, 64, kernel_size=3, stride=1, use_norm=self.batchNorm, norm_func='BN', act_func='lrelu')
        self.conv5 = ConvLayer(64, 64, kernel_size=3, stride=2, use_norm=self.batchNorm, norm_func='BN', act_func='lrelu')
        self.conv5_1 = ConvLayer(64, 64, kernel_size=3, stride=1, use_norm=self.batchNorm, norm_func='BN', act_func='lrelu')
        self.conv6 = ConvLayer(64, 1024, kernel_size=3, stride=2, use_norm=self.batchNorm, norm_func='BN', act_func='lrelu')
        # Comput the shape based on diff image size
        __tmp = Variable(torch.zeros(1, 6, par.img_w, par.img_h))
        __tmp = self.encode_image(__tmp)
