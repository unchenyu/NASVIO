...
import deepvio.tools.transformations as tr
from deepvio.tools.pose_evaluation_utils import quat_pose_to_mat
...
class kittiOdomEval():
    ...
    def loadDecisions(self, file_name):
        '''
            Each line in the file should follow one of the following structures
            (1) idx pose(3x4 matrix in terms of 12 numbers)
            (2) pose(3x4 matrix in terms of 12 numbers)
    ...
    def calcRTEErrors(self, poses_gt, poses_result):
        err = []
        # for all start positions do        
        for first_frame in range(len(poses_gt)-1):
            
            # compute rotational and translational errors, relative pose error (RPE)
            pose_delta_gt = np.dot(np.linalg.inv(poses_gt[first_frame]), poses_gt[first_frame+1])
            pose_delta_result = np.dot(np.linalg.inv(poses_result[first_frame]), poses_result[first_frame+1])
            pose_error = np.dot(np.linalg.inv(pose_delta_result), pose_delta_gt)

            r_err = self.rotationError(pose_error)
            t_err = self.translationError(pose_error)

            err.append([None, r_err, t_err])
        return err
    
    def calcATEErrors(self, poses_gt, poses_result):
        err = []
        # for all start positions do        
        for first_frame in range(1, len(poses_gt)):
            # compute rotational and translational errors, relative pose error (RPE)
            pose_error = np.dot(np.linalg.inv(poses_result[first_frame]), poses_gt[first_frame])
            t_err = self.translationError(pose_error)
            err.append([None, t_err, t_err])
        return err
    ...
    def calcSpeed(self, poses_gt, poses_result):
        ...
        for first_frame in range(len(poses_gt)-1):
            ...
            speed_err_gt = self.rotationError(pose_error_gt)
            speed_err_est = self.rotationError(pose_error_est)
            speed_err_gt_list.append(speed_err_gt)
            speed_err_est_list.append(speed_err_est)
        return speed_err_gt_list, speed_err_est_list

    def calcAngleSpeed(self, poses_gt, poses_result):
        speed_err_gt_list = []
        speed_err_est_list = []
        # for all start positions do        
        for first_frame in range(len(poses_gt)-1):
    ...
    def plotdecPath_2D(self, seq, poses_gt, poses_result, plot_path_dir, decision):
        '''
            plot path in XY, XZ and YZ plane
        '''
        fontsize_ = 10
        ...
        for st in range(0, len(decision)):
            c_out = (0.1, decision[st]*0.9, 0.1)
            plt.scatter([x_pred[st]], [z_pred[st]], marker='.', color=c_out)
        ...
        png_title = "{}_decision".format(seq)
        plt.savefig(plot_path_dir +  "/" + png_title + ".png", bbox_inches='tight', pad_inches=0.1)
        pdf = matplotlib.backends.backend_pdf.PdfPages(plot_path_dir +  "/" + png_title + ".pdf")        
        fig.tight_layout()
        pdf.savefig(fig)  
    ...
    def eval(self, toCameraCoord):
        ...
        for seq in self.eval_seqs:
            ...
            dec_file_name = self.result_dir + '/out_{}_dec.txt'.format(seq)
            ...
            dec_result = self.loadDecisions(dec_file_name)
            ...
            ATE_err = self.calcATEErrors(poses_gt, poses_result)
            RTE_err = self.calcRTEErrors(poses_gt, poses_result)
            total_err += seq_err
            speed_gt, speed_est = self.calcSpeed(poses_gt, poses_result) 
            sio.savemat('speed_'+str(seq)+'.mat', {'gt': np.array(speed_gt), 'est': np.array(speed_est)})
            speed_gt, speed_est = self.calcAngleSpeed(poses_gt, poses_result) 
            sio.savemat('Angle_speed_'+str(seq)+'.mat', {'gt': np.array(speed_gt), 'est': np.array(speed_est)})
            # ----------------------------------------------------------------------
            ...
            rte_t_err, rte_r_err = self.computeOverallErr(RTE_err)
            ate_t_err, _ = self.computeOverallErr(ATE_err)
                        
            vector = np.vectorize(np.float)
            x = vector(np.array(RTE_err)[:, 1:])

            sio.savemat(seq + '_RTE.mat', {'RTE': x})
            print ("\nSequence: " + str(seq))
            print ('Distance (m): %d' % self.distance)
            print ('Max speed (km/h): %d' % (self.max_speed*3.6))
            ...
            print ("Relative translation error (m): {0:.4f}\n".format(rte_t_err))
            print ("Relative rotational error (deg): {0:.4f}\n".format(rte_r_err/np.pi * 180))
            print ("Absolute RMSE (m): {0:.4f}\n".format(ate_t_err))
            ...
            self.ave_t_err = ave_t_err
            self.ave_r_err = ave_r_err/np.pi * 180
            self.ate_t_err = ate_t_err
            ...
            #self.plotdecPath_2D(seq, poses_gt, poses_result, eva_seq_dir, dec_result)
            plt.close('all')
